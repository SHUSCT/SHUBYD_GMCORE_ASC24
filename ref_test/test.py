...
def test(file_ref, file_test):
    ...
    if 'adv_dc4' in file_ref:
        ...
        if sumref / item < 0.01 and flag == True:
            flag = True
        else:
...
if __name__ == "__main__":
    folder_path = './ref_data' #这里请放置test_bed的路径，也就是说这个路径名以test_bed结束
    ...
    if len(sys.argv) > 1:
        ...
        if test_case in file_list.keys():
            ref_file = './ref_data/' + test_case + '/' + file_list[test_case]
            test_file = folder_path + '/' + test_case + '/' + file_list[test_case]
            test(ref_file, test_file)
            print('Done!')
        else:
            print("文件名参数传递错误！")
    else:
