      ...
      do k = ks, ke
        ...
        do j = mesh%half_jds, mesh%half_jde
          do i = mesh%full_ids, mesh%full_ide - 2, 2
            ...
            if (abs(cfly%d(i+1,j,k)) < 1.0e-16_r8) then
              mfy%d(i+1,j,k) = 0
            else if (cfly%d(i+1,j,k) > 0) then
              ju = j
              call ppm(my%d(i+1,ju-2,k), my%d(i+1,ju-1,k), my%d(i+1,ju,k), my%d(i+1,ju+1,k), my%d(i+1,ju+2,k), ml, dm, m6)
              s1 = 1 - cfly%d(i+1,j,k)
              s2 = 1
              ds1 = s2    - s1
              ds2 = s2**2 - s1**2
              ds3 = s2**3 - s1**3
              mfy%d(i+1,j,k) =  v%d(i+1,j,k) * (ml * ds1 + 0.5_r8 * dm * ds2 + m6 * (ds2 / 2.0_r8 - ds3 / 3.0_r8)) / cfly%d(i+1,j,k)
            else if (cfly%d(i+1,j,k) < 0) then
              ju = j + 1
              call ppm(my%d(i+1,ju-2,k), my%d(i+1,ju-1,k), my%d(i+1,ju,k), my%d(i+1,ju+1,k), my%d(i+1,ju+2,k), ml, dm, m6)
              s1 = 0
