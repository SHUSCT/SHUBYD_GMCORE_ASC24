      ...
      do k = mesh%full_kds, mesh%full_kde
        do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
          do i = mesh%full_ids, mesh%full_ide - 8, 8
            ...
            dmf%d(i+2,j,k) = ((                     &
              mfx%d(i+2,j,k) - mfx%d(i+1,j,k)       &
            ) * mesh%le_lon(j) + (                  &
              mfy%d(i+2,j  ,k) * mesh%le_lat(j  ) - &
              mfy%d(i+2,j-1,k) * mesh%le_lat(j-1)   &
            )) / mesh%area_cell(j)
            dmf%d(i+3,j,k) = ((                     &
              mfx%d(i+3,j,k) - mfx%d(i+2,j,k)       &
            ) * mesh%le_lon(j) + (                  &
              mfy%d(i+3,j  ,k) * mesh%le_lat(j  ) - &
              mfy%d(i+3,j-1,k) * mesh%le_lat(j-1)   &
            )) / mesh%area_cell(j)
            dmf%d(i+4,j,k) = ((                     &
              mfx%d(i+4,j,k) - mfx%d(i+3,j,k)       &
            ) * mesh%le_lon(j) + (                  &
              mfy%d(i+4,j  ,k) * mesh%le_lat(j  ) - &
              mfy%d(i+4,j-1,k) * mesh%le_lat(j-1)   &
            )) / mesh%area_cell(j)
            dmf%d(i+5,j,k) = ((                     &
              mfx%d(i+5,j,k) - mfx%d(i+4,j,k)       &
            ) * mesh%le_lon(j) + (                  &
              mfy%d(i+5,j  ,k) * mesh%le_lat(j  ) - &
              mfy%d(i+5,j-1,k) * mesh%le_lat(j-1)   &
            )) / mesh%area_cell(j)
            dmf%d(i+6,j,k) = ((                     &
              mfx%d(i+6,j,k) - mfx%d(i+5,j,k)       &
            ) * mesh%le_lon(j) + (                  &
              mfy%d(i+6,j  ,k) * mesh%le_lat(j  ) - &
              mfy%d(i+6,j-1,k) * mesh%le_lat(j-1)   &
            )) / mesh%area_cell(j)
            dmf%d(i+7,j,k) = ((                     &
              mfx%d(i+7,j,k) - mfx%d(i+6,j,k)       &
            ) * mesh%le_lon(j) + (                  &
              mfy%d(i+7,j  ,k) * mesh%le_lat(j  ) - &
